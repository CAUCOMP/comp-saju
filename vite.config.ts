...
function readJsonBody(
  req: IncomingMessage,
): Promise<LocalFortuneRequest | undefined> {
    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        ...
        try {
          ...
          const { default: handler } = await import("./api/fortune.ts")
          ...
          res.end(JSON.stringify({ error: "fortune_unavailable" }))
        }
      })
    },
  }
